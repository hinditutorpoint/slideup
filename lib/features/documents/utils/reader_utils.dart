...
import 'package:hive_flutter/hive_flutter.dart'; // Changed from secure storage
...
class ReaderStorageManager {
  ...
  // Memory Caches (for sync access matching original API)
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      // Populate memory caches from Boxes
      _loadPositionsCache();
      _loadBookmarksCache();
  ...
  void _loadPositionsCache() {
    ...
    for (var key in _positionsBox.keys) {
      ...
      if (raw != null) {
        try {
          // Hive stores Map<dynamic, dynamic>, need to cast for fromJson
  ...
  Future<List<Bookmark>> getBookmarks(String identifier) async {
    ...
    // Sort by page (create copy to sort)
